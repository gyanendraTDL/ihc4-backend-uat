...
public class CreateTicketService {
	...
	public void createTicket(int feedbackID, Feedback fb) {
		...
		List<UserMap> userMapData = userMap.findByFormTypeAndBranchIDAndLevelAndCompanyID(fb.getType(),
				fb.getBranchID(), 1, fb.getCompanyID());
			...
			try {
				for (int i = 0; i < factorQues.size() - 1; i++) {
					...
					if (val != 0 && val <= factorTicket.getTicketRating()) {
						...
						int factorNumber = 0;
						String factorName = "";
						int ticketUnique = 0;// based on this id (ie factor primary key) escaltion is fetched instead of
												// factorname
...
//									else
							// generateSubserviceTicket(factorTicket, userMap, fb, escalationData,
							// ticketList);
						} else {
							if (factorTicket.getIsHeader() == 1) {
								...
								for (int j = 0; j < subQues.length; j++) {
									...
									if (subfactorVal <= subfactorTicket.getTicketRating() && subfactorVal != 0) {
										if (fb.getCompanyID() != 1
												|| (fb.getCompanyID() == 1 && fb.getChannelID() != 6)) {
							...
							if (factorName != null && factorTicket.getWardTicket() == 1)
								umap = userMap.findEscalationByWard(fb.getCompanyID(), fb.getBranchID(),
										(fb.getWardNo() == null) ? "" : fb.getWardNo(), ticketUnique, fb.getType());
							else
...
//									}
							...
							String fbCmts = "(Poor rating is for " + factorName + " " + subfactorName + ")"
									+ serviceCmt;
							if (!factorName.equalsIgnoreCase("OVERALL_EXPERIENCE"))
								createTicket(fb.getId(), (umap != null) ? umap.getUserID() : 0, factorNumber,
										fb.getType(), fb.getCompanyID(), fb.getBranchID(), factorName, fbCmts,
										fb.getCountryID(), fb, umap, subVal, ticketList, 0, factorTicket);
						}
					}
				}
			} catch (Exception e) {
			...
			try {
				if ((fb.getRating() <= factorTicket.getTicketRating() && fb.getRating() != -1)
						|| (fb.getFeedback() == 0 && fb.getFeedback() != -1)) {
					// log.info(fb.toString()+"=="+"OVERALL_EXPERIENCE from create Ticket - 2.2"+"==
					// Rating:"+fb.getRating()+"== :"+fb.getFeedback());
					...
					createTicket(fb.getId(), (umap != null) ? umap.getUserID() : 0, factorTicket.getId(), fb.getType(),
							fb.getCompanyID(), fb.getBranchID(), "OVERALL_EXPERIENCE", "(Overall Rating is Poor)",
							fb.getCountryID(), fb, umap, null, ticketList, 0, null);
			...
			if (c.isPresent() && c.get().getAutoRouting() == 1 && fb.getComments() != null) {
				...
				List<AutoRoutingCriteria> autoWords = autoRouting.findByCompanyIDAndBranchID(fb.getCompanyID(),
						fb.getBranchID());
				autoWords.forEach(word -> {
					if (fb.getComments().toUpperCase().contains(word.getWord().toUpperCase())) {
						createTicket(fb.getId(), (word != null) ? word.getEscalationID() : 0, 0, fb.getType(),
								fb.getCompanyID(), fb.getBranchID(), "Word_Based",
								"(Word_Based - " + word.getWord().toUpperCase() + ")", fb.getCountryID(), fb, null,
								word.getWord().toUpperCase(), ticketList, 0, null);
					}
	...
	Integer createTicket(int feedbackID, int escalationID, int factorNumber, String type, int companyID, int branchID,
			String FactorName, String fbcomments, int countryID, Feedback fb, UserMap umap, String subFactorName,
			List<Ticket> ticketList, int subFactorNumber, FactorName factorTicket) {
	...
	private void createdEscalation(int ticketID, int escalationID, Feedback fb, UserMap umap, List<Ticket> ticketList,
			FactorName factorTicket) {
		/*
		...
		email.setMailToAddress(
				new String[] { "shailja@thedecisionlabs.com", "support@thedecisionlabs.com", "ved@thedecisionlabs.com",
						"anupam.c@thedecisionlabs.com", "abhay@thedecisionlabs.com", "maria@cempia.com" });
		...
		if (emailDomain.getSendmsg() == 1) {
			...
			if (fb.getCompanyID() == 116) {
				// smsLink=" Dear "+user.getUsername() +" the following ticket No
				// "+t.getTicketNumber()+" is for your kind action. Please click on the SPS
				// hospital link for additional details and resolution status (Issue Active). is
				// for your kind action.
				// https://cempia.com/"+emailDomain.getPathName()+"/tktRsln?_1="+emailDomain.getId()+"_"+fb.getBranchID()+"_"+t.getEscalationID()+"_"+t.getId()+"
				// SPS hospital.";
				smsLink = "Dear " + user.getUsername() + ", the following ticket No " + t.getTicketNumber()
						+ " is for your kind action. Please click on the SPS hospital link for additional details and resolution status (Issue Active). https://cempia.com/"
						+ emailDomain.getPathName() + "/tktRsln?_1=" + emailDomain.getId() + "_" + fb.getBranchID()
						+ "_" + t.getEscalationID() + "_" + t.getId() + "  SPS hospital";
			} else if (fb.getCompanyID() == 115) {
				smsLink = " Dear" + user.getUsername() + ", the following ticket no " + t.getTicketNumber()
						+ " is for your kind action. Please click on the link for additional details and resolution. https://cempia.com/"
						+ emailDomain.getPathName() + "/tktRsln?_1=" + emailDomain.getId() + "-" + fb.getBranchID()
						+ "-" + t.getEscalationID() + "-" + t.getId() + "  Dr.L H Hiranandani hospital";
				template = "1107162064719624312";
			} else if (fb.getCompanyID() == 117) {
				smsLink = " Dear" + user.getUsername() + ", the following ticket no " + t.getTicketNumber()
						+ " is for your kind action. Please click on the link for additional details and resolution. https://cempia.com/"
						+ emailDomain.getPathName() + "/tktRsln?_1=" + emailDomain.getId() + "-" + fb.getBranchID()
						+ "-" + t.getEscalationID() + "-" + t.getId() + "  Sparsh hospital";
			} else if (fb.getCompanyID() == 119) {
				smsLink = " Dear " + user.getUsername() + ", the following ticket no " + t.getTicketNumber()
						+ " is for your kind action. Please click on the link for additional details and resolution. https://cempia.com/"
						+ emailDomain.getPathName() + "/tktRsln?_1=" + emailDomain.getId() + "-" + fb.getBranchID()
						+ "-" + t.getEscalationID() + "-" + t.getId() + " Bangalore Baptist Hospital";
			} else if (fb.getCompanyID() == 118 || fb.getCompanyID() == 123) {
				smsLink = "Dear " + user.getUsername() + ", the following ticket No " + t.getTicketNumber()
						+ " is for your kind action. Please click on the link for additional details and resolution.  https://cempia.com/"
						+ emailDomain.getPathName() + "/tktRsln?a1=" + emailDomain.getId() + "-" + fb.getBranchID()
						+ "-" + t.getEscalationID() + "-" + t.getId() + " RUBYHALL Team.";
			} else if (fb.getCompanyID() == 120) {
				smsLink = "Dear " + user.getUsername() + ", the following ticket No " + t.getTicketNumber()
						+ " is for your kind action. Please click on the link for additional details and resolution. https://cempia.com/"
						+ emailDomain.getPathName() + "/tktRsln?_1=" + emailDomain.getId() + "-" + fb.getBranchID()
						+ "-" + t.getEscalationID() + "-" + t.getId() + " BhaktiVedanta Hospital";
			} else if (fb.getCompanyID() == 121) {
				if (fb.getBranchID() == 10) {
					smsLink = "Dear " + user.getUsername() + ", the following ticket No " + t.getTicketNumber()
							+ " is for your kind action. Please click on the link for additional details and resolution. https://cempia.com/"
							+ emailDomain.getPathName() + "/tktRsln?_1=" + emailDomain.getId() + "_" + fb.getBranchID()
							+ "_" + t.getEscalationID() + "_" + t.getId()
				...
				smsLink = " Dear " + user.getUsername() + ", The following ticket on " + t.getFactorName()
						+ " is for your kind action(generated from " + ChannelName
						+ "). Please click on the link for additional details and resolution https://cempia.com/"
						+ emailDomain.getPathName() + "/tktRsln?_1=" + emailDomain.getId() + "_" + fb.getBranchID()
						+ "_" + t.getEscalationID() + "_" + t.getId() + " - Fernandez Hospital";

			} else if (fb.getCompanyID() == 126) {
				smsLink = " Dear" + user.getUsername() + ",CEMPIA Ticket No(" + t.getTicketNumber()
						+ ") has been assigned to you for " + t.getFeedbackComments() + "from Patient - "
						+ AESencrp.decrypt(fb.getName()) + "(" + AESencrp.decrypt(fb.getContactNo())
						+ ") to take action click on https://cempia.com/" + emailDomain.getPathName() + "/tktRsln?_1="
						+ emailDomain.getId() + "-" + fb.getBranchID() + "-" + t.getEscalationID() + "-" + t.getId();
			} else
				smsLink = " Dear " + user.getUsername() + " the following ticket on " + t.getFactorName()
						+ " is for your kind action. Please click on the link for additional details and resolution https://cempia.com/"
						+ emailDomain.getPathName() + "/tktRsln?_1=" + emailDomain.getId() + "_" + fb.getBranchID()
					...
					if (ccNumList != null) {
						for (int i = 0; i < ccNumList.length; i++) {
							smsLink = " Dear " + user.getUsername() + ", The following ticket on " + t.getFactorName()
									+ " is for your kind action(generated from " + ChannelName
									+ "). Please click on the link for additional details and resolution https://cempia.com/"
									+ emailDomain.getPathName() + "/tktRsln?_1=" + emailDomain.getId() + "_"
									+ fb.getBranchID() + "_" + t.getEscalationID() + "_" + t.getId()
									+ " - Fernandez Hospital";
